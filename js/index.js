...
function rats() {
    const rat_srcs = ['img/rats/rat1.png', 'img/rats/rat2.png', 'img/rats/rat3.png']

    for (let i = 0; i < 200; i++) {
        const rat = document.createElement('img')
        rat.style.width = '100px'
        // rat.animationDuration = '3s'

        rat.src = rat_srcs[Math.floor(Math.random() * rat_srcs.length)]

        const div = document.createElement('div')
        div.style.position = 'absolute'
        div.style.top = (-Math.random() * 100).toString() + 'px'
        div.style.left = Math.floor(Math.random() * screen.width) + 'px'
        div.style.animationDuration = (2 + Math.random() * 3).toString() + 's'
        div.style.animationDelay = (2 * Math.random()).toString() + 's'
        div.className = 'rat'
        div.appendChild(rat)
    ...
    setTimeout(function () {
        window.location.href = 'html/rats.html'
    }, 5000)
}
